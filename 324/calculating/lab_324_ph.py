...
def main():
    x = np.array([1.0, 0.82, 0.44, 0.31, 0.25])
    y = np.array([2.59, 2.52, 0.95, 0.87, 0.65])

    x_approx = np.linspace(x[0], x[x.size - 1], x.size)
    y_approx = k_only(x, y) * x_approx

    sigma_R = 0.02
    R_arr = np.array([1, 1.1, 1.5, 1.8, 2])
    cross_x = np.array([])
    ...
    u_k = np.array([6.0, 3.3, 6.0, 3.0, 3.0])
    u_kn = np.array([0.5, 0.5, 0.1, 0.35, 0.25])
    n = np.array([4, 3, 4, 2, 2])
    div = u_k / u_kn
    log = np.log(div)
    teta = log / n
    ...
    plot(x, y, "ph", x_approx = x_approx, y_approx = y_approx, x_cross=cross_x, y_cross=cross_y )
    return 
